  ...
  return (
    <div className="relative w-full px-8 py-6 mx-auto bg-white rounded-lg shadow-md shadow-blue-200/50 hover:shadow-lg hover:shadow-violet-200/50">
      <div className="flex items-center justify-between">
        {/* @ts-ignore */}
        <time dateTime={format(new Date(post?.date), 'yyyy-mm-dd h:i')} className="font-light text-slate-400">{format(new Date(post?.date), 'MMM yyyy')}</time>
        <span className={`p-2 font-bold text-gray-100 bg-${post.category.color}-500 rounded-full hover:bg-violet-600`}>
      ...
      <div className="flex items-center justify-between mt-5">
        <a href={post.url} {...post.url && {target: '_blank'}} className="after:absolute after:inset-0 text-blue-500 hover:underline flex flex-row items-center" rel="noreferrer">
          Read more
        ...
        <div>
          <a href="#" className="flex items-center ">
            ...
            <h1 className="font-bold text-gray-700 hover:underline">{post.author.fullName}</h1>
          </a></div>
      </div>
    </div>
  )
